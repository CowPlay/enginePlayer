...
namespace irrgame
{
	namespace video
	{
		...
		//! Default constructor
		CNullDriver::CNullDriver(IEnginePlayer* player) :
		...
		bool CNullDriver::checkPrimitiveCount(u32 prmCount) const
		{
			...
			if (prmCount > m)
			{
				// Could not draw triangles, too many primitives
				IRR_ASSERT(false);
				return false;
			}
		...
		void CNullDriver::draw2DVertexPrimitiveList(const void* vertices, u32 vertexCount, const void* indexList, u32 primitiveCount, EVertexType vType, scene::EPrimitiveType pType, EIndexType iType)
		{
		...
		void CNullDriver::drawVertexPrimitiveList(const void* vertices, u32 vertexCount, const void* indexList, u32 primitiveCount, EVertexType vType, scene::EPrimitiveType pType, EIndexType iType)
		{
			if ((iType==EIT_16BIT) && (vertexCount>65536)) {
				//Too many vertices for 16bit index type, render artifacts may occur
				IRR_ASSERT(false);
