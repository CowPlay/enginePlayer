...
namespace irrgame
{
	namespace video
	{
		...
		{
			const f32 ogl_ver = core::StaticConverter::convertToFloat(
					reinterpret_cast<const c8*>(glGetString(GL_VERSION)));
			Version = static_cast<u16>(core::StaticMath::floor_(ogl_ver) * 100
					+ core::StaticMath::roundi(
							core::StaticMath::fract(ogl_ver) * 10.0f));

			//TODO: check version OpenGL and tell to customer about wrong version
			IRR_ASSERT(Version >= 200);

			const c8* t =
					reinterpret_cast<const c8*>(glGetString(GL_EXTENSIONS));
					...
					|| Version >= 200)
			{
				...
				{
					const f32 sl_ver = core::StaticConverter::convertToFloat(
							reinterpret_cast<const c8*>(shaderVersion));
					ShaderLanguageVersion =
							static_cast<u16>(core::StaticMath::floor_(sl_ver)
									* 100
									+ core::StaticMath::roundi(
											core::StaticMath::fract(sl_ver)
													* 10.0f));
				}
			}
			...
			MaxTextureUnits = core::StaticMath::mini(MaxTextureUnits,
					static_cast<u8>(MaterialMaxTextures));

		}

		//! Disable feature
		void SharedOpenGLExtensionHandler::disableFeature(
				EVideoDriverFeatures feature)
		{
		...
		void SharedOpenGLExtensionHandler::extGlActiveTexture(GLenum texture)
		{
		...
		//! Selecting current texture coordinate set for texture mapping
		void SharedOpenGLExtensionHandler::extGlClientActiveTexture(
				GLenum texture)
		{
			if (MultiTextureExtension)
			{
